                ...
                LazyRow(
                    ...
                    items(listForecast!!) { currentItem ->
                        currentItem.let { item ->
                            ...
                            item.main?.let { main ->
                                temp = if (main == null) Constants.NA else "${main.temp}°C"
                            }

                            item.weather?.let { weather ->
                                icon = if (weather == null) Constants.NA else buildingIconUrl(
                                    weather[0].icon!!,
                                    isBigSize = false
                                )
                            }
                            ...
                            item.dt?.let { dateTime ->
                                time = if (dateTime == null) Constants.NA else timeStampToHumanDate(
                                    dateTime.toLong(),
                                    "EEE dd-MMM"
                                )
                            }
...
fun ForecastTitle(temp: String, image: String, time: String) {
    ...
    Card(
        ...
        Column(
            ...
            Text(text = temp.ifEmpty { Constants.NA }, color = Color.White)
            AsyncImage(
            ...
            Text(text = time.ifEmpty { Constants.NA }, color = Color.White)
